...
let serieVoiceDigits = "";
let serieVoiceTimer = null;
...
function extractSpokenDigits(text) {
  ...
  return safeText(text)
    ...
    .flatMap((token) => {
      if (!token) return [];
      if (digitWords.has(token)) return [digitWords.get(token)];
...
function finishSerieVoiceInput() {
  clearTimeout(serieVoiceTimer);
  serieVoiceTimer = null;
  const recognition = serieVoiceRecognition;
  serieVoiceRecognition = null;
  if (recognition) {
    try { recognition.stop(); } catch {}
  }
  $("voiceSerieBtn").classList.remove("listening");
  if (!serieVoiceDigits) {
    $("voiceSerieStatus").textContent = "No he detectado números. Pulsa Dictar e inténtalo de nuevo.";
    return;
  }
  $("numeroSerie").value = serieVoiceDigits;
  $("voiceSerieStatus").textContent = `Número guardado: ${serieVoiceDigits}`;
}

function scheduleSerieVoiceFinish() {
  clearTimeout(serieVoiceTimer);
  serieVoiceTimer = setTimeout(finishSerieVoiceInput, 5000);
  $("voiceSerieStatus").textContent = `Escuchado: ${serieVoiceDigits}. Guardaré en 5 segundos si no dices más números.`;
}

function startSerieVoiceInput() {
  const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!SpeechRecognition) {
    $("voiceSerieStatus").textContent = "Este navegador no permite dictado de voz. Puedes escribir el número manualmente.";
    alert("Este navegador no permite dictado de voz. Prueba en Chrome o Edge, o escribe el número manualmente.");
    return;
  }
  clearTimeout(serieVoiceTimer);
  if (serieVoiceRecognition) {
    try { serieVoiceRecognition.abort(); } catch {}
  }
  serieVoiceDigits = "";
  ...
  serieVoiceRecognition.onresult = (event) => {
    for (let index = event.resultIndex; index < event.results.length; index += 1) {
      ...
      const digits = extractSpokenDigits(event.results[index][0]?.transcript || "");
      if (!digits) continue;
      serieVoiceDigits += digits;
      scheduleSerieVoiceFinish();
    }
  };
  serieVoiceRecognition.onerror = () => {
    $("voiceSerieStatus").textContent = serieVoiceDigits
      ? `Escuchado: ${serieVoiceDigits}. Guardaré en 5 segundos si no dices más números.`
      : "No he detectado números. Pulsa Dictar e inténtalo de nuevo.";
  };
  serieVoiceRecognition.onend = () => {
    $("voiceSerieBtn").classList.remove("listening");
    if (serieVoiceRecognition && serieVoiceDigits && !serieVoiceTimer) scheduleSerieVoiceFinish();
  };
  $("voiceSerieBtn").classList.add("listening");
  $("voiceSerieStatus").textContent = "Escuchando. Di los números uno a uno.";
  serieVoiceRecognition.start();
}

function openEditableList(input) {
  input.focus();
...
async function downloadExcel() {
  ...
  sheet.getRow(1).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFEF1D1D" } };
...
function bindEvents() {
  ...
  ["modelo", "fechaFabricacion", "fechaProximoRetimbrado"].forEach((id) => {
    $(id).addEventListener("focus", () => openEditableList($(id)));
    $(id).addEventListener("click", () => openEditableList($(id)));
  });
